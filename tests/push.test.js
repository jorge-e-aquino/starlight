...
const earlyExam = candidatesForDay(schema, { items: {} }, '2026-09-22').find((c) => c.itemId === 'mgt2250-test1');
assert.equal(earlyExam.kind, 'regular');
assert.match(earlyExam.action, /sources disagree/);
const blockedGate = candidatesForDay(schema, { items: { 'econ-honorlock': { externalBlock: true } } }, '2026-09-29').find((c) => c.itemId === 'econ-honorlock');
assert.equal(blockedGate.kind, 'regular');
assert.match(blockedGate.action, /Midterm 1 depends on this/);
assert.match(blockedGate.action, /Check the blocker/);
assert.match(blockedGate.action, /listed exam date/);
assert.equal(candidatesForDay(schema, { items: { 'econ-honorlock': { externalBlock: true } } }, '2026-09-29').some((c) => c.itemId === 'econ-midterm1'), false);
const clearedGate = candidatesForDay(schema, { items: { 'econ-honorlock': { doneAt: 1 } } }, '2026-10-01').find((c) => c.itemId === 'econ-midterm1');
assert.match(clearedGate.action, /unchecked/);
...
assert.equal(candidatesForDay(schema, confirmed, '2026-10-02').filter((c) => c.itemId === exam.id).length, 1);
